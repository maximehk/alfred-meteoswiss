...
- Results for each prefix are cached for 1 day via Alfred-PyWorkflow's `cached_data`. Queries longer than 2 characters filter the cached list locally and never hit the network, keeping the workflow fast and the MeteoSwiss servers happy.
